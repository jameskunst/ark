...
static int aw8697_haptic_rtp_init(struct aw8697 *aw8697)
{
    ...
    while((!aw8697_haptic_rtp_get_fifo_afi(aw8697)) &&
        ...
        if(aw8697->rtp_cnt == aw8697_rtp->len || !buf_len) {
            pr_debug("%s: rtp update complete,buf_len:%d\n", __func__, buf_len);
            aw8697->rtp_cnt = 0;
            mutex_unlock(&aw8697->rtp_lock);
		...
		const char *buf, size_t count)
{
	...
	if (count != sizeof(struct tp_input_info)) {
		pr_debug("%s: unmatch buf len, node_len=%d, struct len=%d\n",
		...
		char *buf)
{
	...
	if (!p_ai_data) {
		pr_debug("%s: p_ai_data error allocating memory\n", __func__);
		mutex_unlock(&aw8697->haptic_audio.lock);
		return len;
	}
		...
		const char *buf, size_t count)
{
	...
	if (!ai_tz) {
		pr_debug("%s: ai_tz error allocating memory\n", __func__);
		mutex_unlock(&aw8697->haptic_audio.lock);
		return count;
	}
	...
	if (!ai_tz_info) {
		pr_debug("%s: ai_tz_info error allocating memory\n", __func__);
		kfree(ai_tz);
		mutex_unlock(&aw8697->haptic_audio.lock);
		return count;
	}
		...
		unsigned char reg_addr, unsigned char *buf, unsigned int len)
{
	...
	if (ret) {
		pr_debug("%s: couldn't send request, ret=%d\n",
			__func__, ret);
		return ret;
	}
